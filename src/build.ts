...
function normalizeTmdbPosterUrl(value: string): string {
	try {
...
function toPosterUrl(posterPath: string | null): string | null {
	if (! posterPath) {
		return null
	}
	...
	if (! path || lowerPath === 'null' || lowerPath.endsWith('/null')) {
		return null
	}

	if (path.startsWith('http://') || path.startsWith('https://')) {
		return normalizeTmdbPosterUrl(path)
	}

	if (path.startsWith('//')) {
		return normalizeTmdbPosterUrl(`https:${path}`)
	}

	if (path.startsWith('/')) {
		return normalizeTmdbPosterUrl(`${TMDB_IMAGE_BASE}${path}`)
	}
...
async function fetchAndBuildList(env: Env, type: MediaType, language: string): Promise<DiscoveryList | null> {
	...
	const items: DiscoveryItem[] = results.map((result, index) => ({
		...
		poster_path: toPosterUrl(result.poster_path),
	}))
...
export async function buildUpcomingList(env: Env, type: MediaType, language: string, region: string): Promise<UpcomingList | null> {
	const liveKey = `upcoming:${type}:${language}:${region}`
	...
	const list = await fetchAndBuildUpcomingList(env, type, language, region)
...
async function fetchAndBuildUpcomingList(env: Env, type: MediaType, language: string, region: string): Promise<UpcomingList | null> {
	const headers = tmdbHeaders(env.TMDB_API_KEY)
	...
	const upcomingUrl = type === 'movies'
		? (page: number, lang: string) => tmdbUrl.upcomingMovies(page, lang, region, today, ninetyDaysFromNowISO)
		: (page: number, lang: string) => tmdbUrl.upcomingSeries(page, lang, region, today, ninetyDaysFromNowISO)
